...
function getChildren(p) {
  ...
  const children = files
    ...
    .map((file) => {
      ...
      const title = fs
        .readFileSync(path.resolve(p, name), {
          encoding: "utf-8",
        })
        .split("\n")
        .find((str) => {
...
function loadConfig(cf) {
  ...
  children.forEach((child) => {
    ...
    let children = getChildren(
      path.resolve(__dirname, `../../${root}`, childSider)
    );

    // 去重(剔除出已经配置的)
    if (Array.isArray(oldChildren)) {
      children = oldChildren.concat(
        children.filter((c) => {
          return (
            oldChildren.findIndex((v) => {
              return v[0] === c[0];
            }) === -1
          );
        })
...
const childSiderbar = files
  .filter((f) => f.isFile() && f.name !== "index.js")
  .map((v) => {
